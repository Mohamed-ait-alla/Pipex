...
/*   Updated: 2025/02/19 15:30:50 by mait-all         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **env)
{
	...
	int status;
	waitpid(pids[0], &status, 0);  // Wait for the first child
	if (WIFEXITED(status) && WEXITSTATUS(status) != 0) {
    // First child failed, stop further execution
    	exit(WEXITSTATUS(status));
	}
// waitpid(pids[1], NULL, 0);  // Wait for second child
